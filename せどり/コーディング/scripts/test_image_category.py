...
def label_of(keyword, name):
    """検索キーワード（と商品名）から正解ラベルを決める。決められなければ None"""
    text = f"{keyword} {name}".lower()
    kw = (keyword or "").lower()
    # キーワードに種類が書いてあれば、それを正解とする（一番信頼できる）
    if any(w.lower() in kw for w in NON_CLOTHING_WORDS):
        return "服以外"
    if any(w.lower() in kw for w in CLOTHING_WORDS):
        return "服"
    # キーワードがブランド名だけの時は、商品名で判断できる物だけ使う
    if any(w.lower() in text for w in NON_CLOTHING_WORDS):
        return "服以外"
    if any(w.lower() in text for w in CLOTHING_WORDS):
        return "服"
    return None
