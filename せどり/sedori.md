...
- 復旧系: `repair-db`（壊れた指紋を除去して相場DBを圧縮し直す）
...
| 3. 人の目レベルの最終確認 | `verify_ai.py` | 写真2枚をAIに見せて「同じ商品か」を聞く。Gemini（無料枠）優先、無ければAnthropic（有料）。5秒以上の間隔で呼ぶ | 最終確認なしで進む |
...
- `せどり/コーディング/scripts/`: 実際に動くPythonプログラム
  ...
  - 相場DB: `collect_souba.py` / `db_release.py` / `upgrade_embeddings.py` / `discover_brands.py`
  - 購入記録: `purchases.py`
...
| `souba.json` | 手入力の売却実例（予想利益の根拠） | `records[]`: `brand` / `keywords` / `mercari_price` / `memo`、`設定`: 利益通知ライン（既定2,000円）など |
...
- **予想利益**: 相場DBの売却価格 × (1 − メルカリ手数料) − 送料 − 仕入れ値（手数料・送料は `souba.json` の `設定`）
