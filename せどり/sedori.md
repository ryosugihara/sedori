...
## 監視するブランド（4ブランドに集中）
2026-09-16 から、相場DBの収集・新着監視・メルカリ内スキャン・優先スキャンのすべてを **GUCCI・CELINE・Undercover・Saint Laurent** の4ブランドに集中させている。

- 4ブランドとも①優先ブランド扱い（単調な服は『派手か利益が出る物だけ通知』で弾かれる）
- Undercover は仕入れ先の新着監視で「デニム・ジーンズ」だけに絞っている（`only_keywords`）
- 服以外（バッグ・ベルト等）の検索語は外した。CELINE はバッグ中心の検索語から、服（ジャケット・レザー・デニム・シャツ等）の検索語に入れ替えた
- KINDAL・BRING には GUCCI・CELINE の売り場を登録していない（登録には各サイトの売り場名＝`collection` が必要）
- 止めた20ブランドは各 `watch_*.json` の `_停止中のブランド` に、外した検索語は `watch_mercari.json` の `_外した検索語` に控えてある。`brands` / `keywords` に移せば戻せる（新着監視のサイトは、長く止めていたブランドを戻すと、その間の在庫がまとめて新着として通知されるおそれがあるので注意）
- 優先スキャンの対象は `watch_mercari.json` の `優先スキャンのブランド` で決める
...
|---|---|---|---|
| ① 優先ブランド | `profit_only` なし（今は4ブランドすべて） | ブランドが一致した新着のうち、**派手さを示す言葉があるか、相場照合で利益が通知ライン以上**の物を通知（`priority.json` で全部通知に戻せる） | 派手さを示す言葉がある物だけ通知する |
...
## 調査の優先順位（priority.py）
設定は `watchlists/priority.json`。2つの仕組みがある。

**1. 調査しない商品の決め方（3段構え）**
服以外を弾くのに、確実な情報から順に見る。**新着監視・優先スキャン・メルカリ内スキャン・在庫スキャンの4経路すべてに適用**。

| 段 | 判断材料 | 強み・弱み | 設定 |
|---|---|---|---|
| ① | **サイトのカテゴリ**（KINDAL/BRINGのproduct_type・RINKAN・おふもーる） | 商品名が雑でも分類は正しい。**『服』と分類されていれば名前に関係なく調査する**（取りこぼし防止） | `カテゴリで判定する` |
| ② | **名前の言葉** | 「バッグ」「財布」等がはっきり書いてあれば確実。バッグの型名（ミューズトゥ等）も登録済み。ただし名前が雑な出品には効かない | `服以外_調査しない` |
| ③ | **写真そのもの**（CLIPのゼロショット判定） | 名前もカテゴリも無い出品に効く最後の砦。写真の指紋は画像照合で計算済みなので追加費用ゼロ。**実測: 服の取りこぼし0.8%で服以外の90.5%を弾ける**（差0.03） | `写真で判定する` |

さらに `シンプル服_調査しない` で、無地系カテゴリかつ派手さを示す言葉が無い服（黒Tシャツ・黒スキニー等）を除外できる。

⚠️ **やめた方法**: 「服の言葉が1つも無い商品を弾く」（`服と確認できない商品_調査しない`）は既定オフ。安く出ている狙い目の商品は商品名が雑なことが多く、利益の出る商品まで見送るため。

除外件数は診断レポートの「その他」に理由別（服以外／服以外(カテゴリ)／服以外(写真判定:バッグ)／シンプル服）で残る。

**1-2. ①優先ブランドの新着の絞り込み**
①優先ブランド（サンローラン等）の新着は従来すべて通知していたが、単調な服が多く混ざるため、**『派手さを示す言葉がある』か『相場照合で利益が通知ライン以上』のどちらかを満たす物だけ**を通知する（`優先ブランド_派手か利益が出る物だけ通知する`）。商品名が雑でも利益が出るなら通知されるので、狙い目は逃さない。`false` にすると従来通り全部通知する。

⚠️ **写真からの柄判定は採用しない**: CLIPゼロショット・CLIP平均像くらべ・DINOv2平均像くらべの3方式を実データ（派手8,852件/単調10,721件）で測定したが、どれも「派手な服を大量に捨てる」か「単調な服をほとんど弾けない」かで使える設定が無かった（`recon/IMAGE_CATEGORY_CALIB.txt`）。服とバッグの区別（90%成功）と違い、柄の有無は今の道具では見分けられない。派手さの判定は商品名の言葉で行う。

**2. 残った服の中で、調べる順番を点数で決める**
1日の通知上限 `SCAN_MAX` やAIの回数制限を、価値の高い商品に優先して使うための仕組み。**通知する/しないの条件そのものは変えない**。

| 観点 | 点数 | 理由 |
|---|---|---|
| 派手・個性の強いデザイン（プリント/刺繍/総柄/ダメージ/アーカイブ/コラボ等） | **+6** | 高値で売りやすく利益が出やすい |
| 今の季節の服（**15日先取り**） | **+6** | 次の季節を先に仕入れる |
| シンプル（無地系カテゴリで派手ワードが1つも無い） | **−6** | 画像判別が難しく、流通量が多く利益も出にくい |
| 1つ前の季節だけの服 | **−6** | 季節の変わり目に価値が下がるため |

点数は足し算になる（例: 秋物＋派手 = +12 が最優先、夏物＋無地 = −12 が最後回し）。季節は「今日＋`先取り_日数`（既定15日）」の月で判定する（3〜5月=春 / 6〜8月=夏 / 9〜11月=秋 / 12〜2月=冬）。例: 8月17日ごろから秋物が優先される。
...
| `priority.json` | 調査の優先順位（服以外は調査しない・派手デザイン優先・季節15日先取り） | `服以外_調査しない`(true/false) / `低優先カテゴリ`(バッグ等) / `派手デザイン` / `シンプル判定_カテゴリ` / `季節の服`(春夏秋冬) / `先取り_日数`(既定15日) |
