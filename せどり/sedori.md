...
**1. 調査しない商品（3つのスイッチ）**
次のどれかに当てはまる商品は、画像判定も通知も行わずスキップする。**新着監視・優先スキャン・メルカリ内スキャン・在庫スキャンの4経路すべてに適用**（1つでも抜けると通知がすり抜ける）。

| スイッチ | 何を弾くか | 例 |
|---|---|---|
| `服以外_調査しない` | バッグ・靴・財布・帽子・アクセサリー等の言葉がある商品。**バッグの型名も登録済み** | ショルダーバッグ / ミューズトゥ / サックドジュール |
| `服と確認できない商品_調査しない` | 服の言葉（ジャケット・シャツ・パンツ等）が**1つも無い**商品 | 「SAINT LAURENT D02」のような型番だけの商品名 |
| `シンプル服_調査しない` | 無地系カテゴリで、派手さを示す言葉が1つも無い服 | 黒Tシャツ 無地 / スキニーデニム ブラック |

通知が減りすぎた時は、`priority.json` で該当スイッチを `false` にすれば元に戻る。除外件数は診断レポートの「その他」に理由別（服以外／服と確認できない／シンプル服）で残る。
